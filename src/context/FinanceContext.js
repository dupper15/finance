...
function financeReducer(state, action) {
    switch (action.type) {
        ...
        case 'UPDATE_ACCOUNT':
            return {
                ...
                accounts: state.accounts.map(account =>
                    account.account_id === action.payload.account_id ? action.payload : account
                ),
            };
        ...
        case 'DELETE_TRANSACTION':
            return {
                ...
                transactions: state.transactions.filter(transaction =>
                    transaction.transaction_id !== action.payload
                ),
            };
        ...
        case 'UPDATE_SCHEDULED_TRANSACTION':
            return {
                ...
                scheduledTransactions: state.scheduledTransactions.map(scheduled =>
                    scheduled.scheduled_id === action.payload.scheduled_id ? action.payload : scheduled
                ),
            };
        ...
        case 'DELETE_SCHEDULED_TRANSACTION':
            return {
                ...state,
                scheduledTransactions: state.scheduledTransactions.filter(scheduled =>
                    scheduled.scheduled_id !== action.payload
                ),
            };
...
export function FinanceProvider({ children }) {
    ...
    const updateAccount = async (accountId, accountData) => {
        try {
            const updatedAccount = await financeService.updateAccount(accountId, accountData);
    ...
    const loadTransactions = async (filters = {}) => {
        try {
            ...
            dispatch({ type: 'SET_TRANSACTIONS', payload: response.transactions || response });
        } catch (error) {
            setError(error.message);
    ...
    const updateTransaction = async (transactionId, transactionData) => {
        try {
            const updatedTransaction = await financeService.updateTransaction(transactionId, transactionData);
    ...
    const loadScheduledTransactions = async () => {
        try {
            ...
            const scheduledTransactions = await financeService.getScheduledTransactions();
    ...
    const createScheduledTransaction = async (scheduledData) => {
        try {
            const newScheduled = await financeService.createScheduledTransaction(scheduledData);
    ...
    const updateScheduledTransaction = async (scheduledId, scheduledData) => {
        try {
            const updatedScheduled = await financeService.updateScheduledTransaction(scheduledId, scheduledData);
            dispatch({ type: 'UPDATE_SCHEDULED_TRANSACTION', payload: updatedScheduled });
    ...
    const toggleScheduledTransaction = async (scheduledId) => {
        try {
            const updatedScheduled = await financeService.toggleScheduledTransaction(scheduledId);
            dispatch({ type: 'UPDATE_SCHEDULED_TRANSACTION', payload: updatedScheduled });
            return updatedScheduled;
        } catch (error) {
            setError(error.message);
