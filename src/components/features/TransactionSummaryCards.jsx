...
export function TransactionSummaryCards({ summary, accountsData }) {
    const cards = [
        ...
        },
        {
            ...
            color: 'blue',
            change: summary.netAmount >= 0 ? `+${((summary.netAmount / (summary.totalExpenses || 1)) * 100).toFixed(1)}%` : `${((summary.netAmount / (summary.totalExpenses || 1)) * 100).toFixed(1)}%`,
                    ...
                    <div className="p-5">
                        <div className="flex items-center">
                            <div className="flex-shrink-0">
                                <div className={`w-8 h-8 bg-${card.color}-100 rounded-md flex items-center justify-center`}>
                                    {card.icon}
                                </div>
                            </div>
                            <div className="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd className="flex items-baseline">
                                        <div className={`text-2xl font-semibold text-${card.color}-600`}>
